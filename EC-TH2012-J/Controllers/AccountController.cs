...
namespace WebNhaHangOnline.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser() { UserName = model.UserName,
                    PhoneNumber = model.DienThoai, Email = model.Email,
                    DiaChi = model.DiaChi, HoTen = model.HoTen,
                    Avatar = "noavatar.jpg",
                    RoleID = "37877e60-4cd3-4eb3-8a94-a5d881a81446"};
        ...
        public async Task<ActionResult> RegisterAdmin(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser()
                {
                    ...
                    HoTen = model.HoTen,
                    Avatar = "noavatar.jpg",
                    RoleID = "dc481379-dfae-4d9e-93d8-dce55add3258"
                };
        ...
        [AllowAnonymous]
        [ValidateAntiForgeryToken]
        public async Task<ActionResult> RegisterNV(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser()
                {
                    ...
                    HoTen = model.HoTen,
                    Avatar = "noavatar.jpg",
                    RoleID = "8d050225-c36f-4790-bd7b-28bb044d90b1"
                };
                ...
                if (result.Succeeded)
                {
                    ...
                    ManagerObiect.getIntance().userName = model.UserName;
                    SendMailConfirm(user.Id);
                    return RedirectToAction("IndexNV", "Account");
                }
                else
                {
                    AddErrors(result);
        ...
        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult KhachHangThanThien()
        {
            UserModel us = new UserModel();
            ViewBag.Roleseach = new SelectList(us.GetAllRole(), "Id", "Name");
            ViewBag.Role = new SelectList(us.GetAllRole(), "Name", "Name");
        ...
        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult TimUserNV(string key, string email, string hoten, string phone, string quyen, int? page)
        {
            UserModel spm = new UserModel();
            ViewBag.key = key;
            ViewBag.email = email;
            ...
            ViewBag.phone = phone;
            ViewBag.quyen = quyen;
            return PhanTrangNV(spm.SearchUserNV(key, email, hoten, phone, quyen), page, null);
        }
        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult TimUserAdmin(string key, string email, string hoten, string phone, string quyen, int? page)
        {
            UserModel spm = new UserModel();
            ViewBag.key = key;
            ViewBag.email = email;
            ...
            ViewBag.phone = phone;
            ViewBag.quyen = quyen;
            return PhanTrangUser(spm.SearchUserAdmin(key, email, hoten, phone, quyen), page, null);
        }

        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult TimUserKH(string key, string email, string hoten, string phone, string quyen, int? page)
        {
            UserModel spm = new UserModel();
            ViewBag.key = key;
            ViewBag.email = email;
            ...
            ViewBag.phone = phone;
            ViewBag.quyen = quyen;
            return PhanTrangUserKH(spm.SearchUserKH(key, email, hoten, phone, quyen), page, null);
        }

        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult TimUserKHThanThiet(string key, string email, string hoten, string phone, string quyen, int? page)
        {
            UserModel spm = new UserModel();
            ViewBag.key = key;
            ViewBag.email = email;
            ...
            ViewBag.phone = phone;
            ViewBag.quyen = quyen;
            return PhanTrangUserKHThanThiet(spm.SearchUserKH(key, email, hoten, phone, quyen), page, null);
        }
        ...
        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult PhanTrangNV(IQueryable<AspNetUser> lst, int? page, int? pagesize)
        {
            int pageSize = (pagesize ?? 10);
            int pageNumber = (page ?? 1);
            return PartialView("UserListNV", lst.OrderBy(m => m.HoTen).ToPagedList(pageNumber, pageSize));
        }

        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult PhanTrangUserKH(IQueryable<AspNetUser> lst, int? page, int? pagesize)
        {
            int pageSize = (pagesize ?? 10);
            int pageNumber = (page ?? 1);
            return PartialView("UserListKH", lst.OrderBy(m => m.HoTen).ToPagedList(pageNumber, pageSize));
        }


        [AuthLog(Roles = "Quản trị viên")]
        public ActionResult PhanTrangUserKHThanThiet(IQueryable<AspNetUser> lst, int? page, int? pagesize)
        {
            int pageSize = (pagesize ?? 10);
            int pageNumber = (page ?? 1);
            return PartialView("UserListKHThanThiet", lst.OrderBy(m => m.HoTen).ToPagedList(pageNumber, pageSize));
        }
    }
}