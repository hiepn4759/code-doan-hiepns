...
function thongketheotgnew() {
    var froms = $('#fday').val();
    var tos = $('#tday').val();
    $.ajax({
        url: "/ThongKe/ThongKeDTTheoTG_",
        type: 'GET',
        data: { froms: froms, tos: tos },
        success: function (data) {
            var Names = []
            var Qts = []
            for (var i = 0; i < data.length; i++) {
                debugger
                Names.push(data[i].ngaymua);
                Qts.push(data[i].tongtien);
            }
            Highcharts.chart('containerTG', {
                ...
                title: {
                    text: 'Báo cáo tổng tiền bán được của cửa hàng theo ngày'
                },
                xAxis: {
                    categories: Names
                },
                ...
                series: [{
                    name: 'Tổng tiền bán theo ngày của cửa  hàng',
                    data: Qts
                }]
            });
        },
...
function thongketheodhnew() {
    var froms = $('#fdayDH').val();
    var tos = $('#tdayDH').val();
    $.ajax({
        url: "/ThongKe/ThongKeDonHangS",
        type: 'GET',
        data: { froms: froms, tos: tos },
        success: function (data) {
            debugger
            ...
            for (var i = 0; i < data.length; i++) {
                debugger
                Names.push(data[i].TenSP);
                Qts.push(data[i].SL);
            }
            Highcharts.chart('containerDH', {
                chart: {
                    type: 'column'
                },
                title: {
                    text: 'Báo cáo tổng số lượng bán được của sản phẩm theo ngày'
                },
                xAxis: {
                    categories: Names
                },
                ...
                series: [{
                    name: 'Tổng số lượng bán được của sản phẩm của cửa  hàng',
                    data: Qts
                }]
            });
        },
...
function thongketheokm() {
    var froms = $('#fdayKM').val();
    var tos = $('#tdayKM').val();
    $.ajax({
        url: "/ThongKe/ThongKeKhuyenMais",
        type: 'GET',
        data: { froms: froms, tos: tos },
        success: function (data) {
            debugger
            ...
            for (var i = 0; i < data.length; i++) {
                Names.push(data[i].TenSP);
                var QtsEnd = data[i].GiaGoc - data[i].GiaTien;
                Qts.push(QtsEnd);
            }
            Highcharts.chart('containerKM', {
                chart: {
                    type: 'column'
                },
                title: {
...
function timkiemkhachhangAdmin() {
    var key = $('#tendangnhaps').val();
    var email = $('#emails').val();
    var hoten = $('#hotens').val();
    var phone = $('#sodienthoais').val();
...
//nha cung cap
function kiemgiatritrungncc(value, Url, id) {
    if (value)
        $.ajax({
            url: Url,
            data: { key: value },
            success: function (result) {
                var s = $(id).parent();
                if (result) {
                    $('#kiemtratrungNCC').html("Ok");
                    s.find("input").css({ "border": "2px solid green" });
                    $('.alert-info').fadeOut(500);
                }
                else {
                    $('#kiemtratrungNCC').html("Đã được sử dụng");
                    s.find("input").css({ "border": "2px solid red" });
                    $('.alert-info').fadeIn(500);
                }
        ...
        var s = $(id).parent();
        $('#kiemtratrungNCC').html("Không được để trống");
        s.find("input").css({ "border": "2px solid red" });
        $('.alert-info').fadeIn(500);
    }
}
